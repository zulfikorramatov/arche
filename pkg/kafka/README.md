...
Рядом лежит `pkg/kafka_old` — предыдущая реализация на `segmentio/kafka-go`, оставлена как референс.
В ней producer работает в режиме `acks=1` + `Async` (fire-and-forget, без подтверждения), а consumer
использует авто-коммит по таймеру — то есть **не** даёт нужных гарантий. Использовать в новом коде
не нужно.
